...
class Tasks_model extends App_Model
{
    ...
    // Extended statuses for workflow testing
    ...
    const STATUS_ON_HOLD = 9;

    const STATUS_CANCELLED = 10;

    const STATUS_READY_FOR_DEPLOYMENT = 11;

    const STATUS_DEPLOYED = 12;

    // 🔹 Development & Handover Statuses (STATUS_NOT_STARTED = 1 already defined above)
    const STATUS_CODE_REVIEW_PENDING = 14;           // Awaiting peer/lead review
    const STATUS_READY_FOR_TESTING = 15;             // Development complete, handed to QA

    // 🔹 Testing Cycle Statuses
    const STATUS_TESTING_BLOCKED = 17;               // QA blocked due to environment/dependencies
    const STATUS_TESTING_FAILED = 18;                // QA found issues and sent back to Dev
    const STATUS_RE_TESTING = 19;                    // QA re-testing after fixes
    const STATUS_REGRESSION_TESTING = 20;            // Ensuring fixes didn't break other features
    const STATUS_QC_PASSED = 21;                     // All test cases passed

    // 🔹 Approval & Release Statuses
    // const STATUS_AWAITING_UAT = 22;                  // Client/Business User acceptance testing pending
    // const STATUS_UAT_IN_PROGRESS = 23;               // Client is testing the feature
    // const STATUS_UAT_FEEDBACK = 24;                  // Issues reported by client
    // const STATUS_UAT_APPROVED = 25;                  // Cleared by client/business
    const STATUS_READY_FOR_RELEASE = 26;             // Approved for deployment
    const STATUS_DEPLOYED_TO_PRODUCTION = 27;        // Released live
    ...
    {
        $completion_statuses = $this->get_completion_statuses();
        $this->db->where('id IN (SELECT taskid FROM ' . db_prefix() . 'task_assigned WHERE staffid = ' . get_staff_user_id() . ')');
        $this->db->where_not_in('status', $completion_statuses);
    ...
    public function get_statuses()
    {
        $statuses = hooks()->apply_filters('before_get_task_statuses', [
            ...
            // [
            //     'id'             => static::STATUS_IN_PROGRESS_DEV,
            //     'color'          => '#3b82f6',
            ...
            // ],
            [
                'id'             => static::STATUS_CODE_REVIEW_PENDING,
                'color'          => '#9333ea',
            ...
            [
                'id'             => static::STATUS_READY_FOR_TESTING,
                'color'          => '#0ea5e9',
            ...
            // ],
            [
                'id'             => static::STATUS_TESTING_BLOCKED,
                'color'          => '#dc2626',
                'name'           => _l('task_status_17'),
                'order'          => 6,
                'filter_default' => true,
            ],
            [
                'id'             => static::STATUS_TESTING_FAILED,
                'color'          => '#ef4444',
                'name'           => _l('task_status_18'),
                'order'          => 7,
                'filter_default' => true,
            ],
            [
                'id'             => static::STATUS_RE_TESTING,
                'color'          => '#f59e0b',
                'name'           => _l('task_status_19'),
                'order'          => 8,
                'filter_default' => true,
            ],
            [
                'id'             => static::STATUS_REGRESSION_TESTING,
                'color'          => '#8b5cf6',
                'name'           => _l('task_status_20'),
                'order'          => 9,
                'filter_default' => true,
            ],
            [
                'id'             => static::STATUS_QC_PASSED,
                'color'          => '#10b981',
                'name'           => _l('task_status_21'),
                'order'          => 10,
                'filter_default' => true,
            ],
            ...
            // 🔹 Approval & Release
            // [
            //     'id'             => static::STATUS_AWAITING_UAT,
            //     'color'          => '#84cc16',
            //     'name'           => _l('task_status_22'),
            //     'order'          => 11,
            //     'filter_default' => true,
            // ],
            // [
            //     'id'             => static::STATUS_UAT_IN_PROGRESS,
            //     'color'          => '#65a30d',
            //     'name'           => _l('task_status_23'),
            //     'order'          => 12,
            //     'filter_default' => true,
            // ],
            // [
            //     'id'             => static::STATUS_UAT_FEEDBACK,
            //     'color'          => '#ea580c',
            //     'name'           => _l('task_status_24'),
            //     'order'          => 13,
            //     'filter_default' => true,
            // ],
            // [
            //     'id'             => static::STATUS_UAT_APPROVED,
            //     'color'          => '#059669',
            //     'name'           => _l('task_status_25'),
            //     'order'          => 14,
            //     'filter_default' => true,
            // ],
            [
                'id'             => static::STATUS_READY_FOR_RELEASE,
                'color'          => '#06b6d4',
                'name'           => _l('task_status_26'),
                'order'          => 15,
                'filter_default' => true,
            ],
            [
                'id'             => static::STATUS_DEPLOYED_TO_PRODUCTION,
                'color'          => '#047857',
                'name'           => _l('task_status_27'),
                'order'          => 16,
                'filter_default' => true,
            ],

            // 🔹 Legacy/Additional Statuses (keeping for backward compatibility)
            [
            ...
            [
                'id'             => static::STATUS_READY_FOR_DEPLOYMENT,
                'color'          => '#06b6d4',
    ...
    /**
     * Check if a status is considered a completion status
     * @param  int $status_id
     * @return boolean
     */
    public function is_completion_status($status_id)
    {
        $completion_statuses = [
            static::STATUS_COMPLETE,
            static::STATUS_DEPLOYED,
            static::STATUS_DEPLOYED_TO_PRODUCTION,
            static::STATUS_CANCELLED,
            static::STATUS_REJECTED
        ];

        return in_array($status_id, $completion_statuses);
    }
     ...
     * @return array
     */
    public function get_completion_statuses()
    {
        return [
            static::STATUS_COMPLETE,
            static::STATUS_DEPLOYED,
    ...
    /**
     * Check if a status is considered an active work status
     * @param  int $status_id
     * @return boolean
     */
    public function is_active_status($status_id)
    {
        $active_statuses = [
            static::STATUS_NOT_STARTED,
            static::STATUS_IN_PROGRESS,
            static::STATUS_IN_PROGRESS_DEV,
            static::STATUS_CODE_REVIEW_PENDING,
            static::STATUS_READY_FOR_TESTING,
            static::STATUS_IN_TESTING,
            static::STATUS_TESTING_BLOCKED,
            static::STATUS_TESTING_FAILED,
            static::STATUS_RE_TESTING,
            static::STATUS_REGRESSION_TESTING,
            static::STATUS_QC_PASSED,
            // static::STATUS_AWAITING_UAT,
            // static::STATUS_UAT_IN_PROGRESS,
            // static::STATUS_UAT_FEEDBACK,
            // static::STATUS_UAT_APPROVED,
            static::STATUS_READY_FOR_RELEASE,
            static::STATUS_TESTING,
            static::STATUS_AWAITING_FEEDBACK,
            static::STATUS_UNDER_REVIEW,
            static::STATUS_APPROVED,
            static::STATUS_READY_FOR_DEPLOYMENT
        ];

        return in_array($status_id, $active_statuses);
    }
    ...
    public function mark_as($status, $task_id)
    {
        ...
        // If task is already in a completion status, unmark it first
        if ($this->is_completion_status($task->status)) {
            return $this->unmark_complete($task_id, $status);
        }

        $update = ['status' => $status];

        // Set completion date for completion statuses
        if ($this->is_completion_status($status)) {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($this->is_completion_status($status)) {
                ...
                // Stop all timers for completion statuses
                $this->db->where('end_time IS NULL');
            ...
            if ($task->rel_type == 'project') {
                // Use completion activity log for all completion statuses
                $project_activity_log = $this->is_completion_status($status) ? 'project_activity_task_marked_complete' : 'not_project_activity_task_status_changed';

                $project_activity_desc = $task->name;

                // Add status description for non-complete statuses
                if (!$this->is_completion_status($status) || $status != static::STATUS_COMPLETE) {
